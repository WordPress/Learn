...
class PLL_Sync_Post_Model {
	...
	protected function doing_bulk_trash( $post_id ) {
		return 'edit.php' === $GLOBALS['pagenow'] && isset( $_GET['action'], $_GET['post'] ) && in_array( sanitize_key( $_GET['action'] ), array( 'trash', 'untrash' ), true ) && in_array( $post_id, array_map( 'absint', $_GET['post'] ), true ); // phpcs:ignore WordPress.Security.NonceVerification
	}
	 ...
	 *
	 * @phpstan-return array<non-empty-string, int<1, max>>
	 */
	public function get( $post_id ) {
		...
		if ( ! empty( $term ) ) {
			...
			if ( ! empty( $d['sync'][ $lang->slug ] ) ) {
				$keys = array_keys( $d['sync'], $d['sync'][ $lang->slug ], true );
	...
	public function are_synchronized( $post_id, $other_id ) {
		return isset( $this->doing_sync[ $post_id ][ $other_id ] ) || in_array( $other_id, $this->get( $post_id ), true );
	}
	...
	public function current_user_can_synchronize( $post_id, $lang ) {
		...
		if ( ! $user->has_cap( 'edit_post', $post_id ) ) {
			// The user is not allowed to edit this post nor its synchronized translations.
			// See `Modules\Capabilities\Mapper\Post::map()`.
			return false;
		}
		...
		if ( empty( $tr_id ) ) {
			...
			return $user->has_cap( $post_type_object->cap->create_posts );
		}

		// Do we have the right to edit this translation?
		return $user->has_cap( 'edit_post', $tr_id );
	}
}
