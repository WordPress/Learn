...
use WP_Syntex\Polylang_Pro\REST\Translated\Template;
...
add_action(
	...
	function ( $polylang ) {
		...
		// Templare REST API class is required only in a REST context.
		add_action(
			'rest_api_init',
			function () use ( $polylang ) {
				$polylang->rest_api->template = new Template( $polylang->rest_api );
			},
