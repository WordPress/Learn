...
class Sensei_Home_Notices {
	...
	private function get_base_plugin_notice( $plugin_data ) {
		...
		if ( ! empty( $changelog_url ) ) {
			$info_link = [
				'label' => __( 'What\'s new', 'sensei-pro' ),
	...
	private function get_unlicensed_plugin_update_notice( array $plugin_data ):array {
		...
		$notice['message'] = wp_kses(
			sprintf(
				...
				__( 'There is a new version of <strong>%1$s</strong> available (%2$s). Please activate the plugin license in order to proceed with the update process.', 'sensei-pro' ),
				$plugin_name,
				$latest_version
			),
			[ 'strong' => [] ]
	...
	private function get_deactivated_plugin_update_notice( array $plugin_data ) {
		...
		$notice['message'] = wp_kses(
			sprintf(
				...
				__( 'There is a new version of <strong>%1$s</strong> available (%2$s). Please activate the plugin in order to proceed with the update process.', 'sensei-pro' ),
				$plugin_name,
				$latest_version
			),
			[ 'strong' => [] ]
	...
	private function get_plugin_update_notice( array $plugin_data ): array {
		...
		$notice['message'] = wp_kses(
			sprintf(
					// translators: First placeholder is the plugin name and second placeholder is the latest version available.
				__( 'There is a new version of <strong>%1$s</strong> available (%2$s). Please update to ensure you have the latest features and fixes.', 'sensei-pro' ),
				$plugin_name,
				$latest_version
			),
			[ 'strong' => [] ]
